...
class notification_sensor(SensorEntity):
    ...
    async def async_update(self):
        ...
        notification_id = []
        notification_id.append(str(notification.get("id", "")))
        notification_id.append(str(notification.get("serial", "")))
        ...
        # Check state change
        new_state = "-".join(notification_id)
        if self._state != new_state:
            self._attr_value[ATTR_AUTHOR] = notification.get("author", "")
            self._attr_value[ATTR_ID] = "-".join(notification_id)
