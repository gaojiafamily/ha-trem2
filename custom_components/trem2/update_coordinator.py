...
class ExpTechWSManager:
    ...
    async def initialize_route(self, exclude: list | None = None):
        """Random the node for fetching data."""
        if exclude is None:
            api_node = WS_URLS.items()
        else:
            api_node = {k: v for k, v in WS_URLS.items() if k not in exclude}

        self.station, self.base_url = random.choice(list(api_node))
...
class ExpTechHTTPManager:
    ...
    async def fetch_http(self, session: ClientSession):
    ...
    async def fetch_report(self, session: ClientSession, report_id):
        ...
        try:
            ...
            _LOGGER.exception(
                "An unexpected exception occurred fetching the data from report server"
            )
        else:
            if response.ok:
...
class trem2_update_coordinator(DataUpdateCoordinator):
    ...
    async def _initialize(self):
        ...
        await ExpTechHTTPManager.initialize_route()  # Required
        # ECS OR Webscoket
    ...
    async def _async_update_data(self):
        ...
        fetch_data = await ExpTechHTTPManager.fetch_http(self.session)
    ...
    async def _load_report_data(self, eq_data, simulator):
        ...
        # Extract data
        eq: dict = (
            eq_data[0] if isinstance(eq_data, list) and len(eq_data) > 0 else eq_data
        )
        ...
        if eew_time > report_time:
            ...
            try:
                ...
                _LOGGER.error(
                    "Failed fetching data from report server, %s", ex.strerror
                )
            except TimeoutError as ex:
                _LOGGER.error(
                    "Failed fetching data from report server, %s", ex.strerror
                    ...
                    if len(resp) > 0:
                        ...
                        if fetch_report_id not in {"", local_report_id}:
                            self._cached_report_data = (
                                await ExpTechHTTPManager.fetch_report(
                                    self.session,
                                    fetch_report_id,
                                )
                            )
