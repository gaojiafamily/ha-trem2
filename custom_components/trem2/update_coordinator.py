...
class ExpTechWSManager:
    ...
    def log_task_exception(self, task: Task):
...
class Trem2UpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _initialize(self) -> bool:
        ...
        publisher = provider_options.get(_provider)
        if len(publisher) > 0:
    ...
    async def _async_update_data(self):
        ...
        if self.task_manager.initialize_task is None:
            ...
            _LOGGER.error(
                "Update failed, next attempt in %.1f seconds",
    ...
    async def _load_eew_data(self, resp: dict, simulator: dict):
        ...
        try:
            ...
            cached_eew = self._cached_eew_data
            store_eew = self.store_eew
    ...
    async def _load_report_data(self, eq_data: dict, simulator: dict):
        ...
        try:
            ...
            if self._cached_report_data is None:
                report_data = await self.store_report.async_load() or {}
            else:
                report_data = (
                    self._cached_report_data[0]
                    if isinstance(
                        self._cached_report_data,
                        list,
                    )
                    and len(self._cached_report_data) > 0
                    else self._cached_report_data
                )
            ...
            if eew_time > report_time:
                ...
                if abs(self.report_fetch_time - datetime.now().timestamp()) > 300:
                    ...
                    report_data = await self.http_manager.fetch_report(self.session, report_data)
    ...
    async def _save_report(self, data):
        ...
        await self.store_report.async_save(data)
    ...
    def _log_task_exception(task: Task):
        ...
        try:
            ...
            _LOGGER.exception("Error retrieving task exception")
            return
    ...
    async def connection_status(self) -> str:
        ...
        if self.ws_manager.is_running:
            return "http (fallback)" if self.use_http_fallback else "websocket"
