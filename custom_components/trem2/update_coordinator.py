...
class Trem2UpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self):
        ...
        if self.ws_client.state.is_running and self.ws_client.state.subscrib_service:
            resp = await self._websocket_update_data()
        ...
        if not self.state.reinitialize:
            self.state.retry_backoff = 1
            self.http_client.unavailables = []
            self.ws_client.unavailables = []

        # If a retry backoff is required, an exception is raised
        if self.state.retry_backoff > 1:
            new_interval = min(
    ...
    async def _websocket_update_data(self):
    ...
    async def _handle(self, resp: dict) -> list:
    ...
    async def _load_eew_data(self, resp: list, simulator: dict) -> dict:
    ...
    def server_status(self, **kwargs):
        ...
        api_node = (
            self.http_client.api_node
            if not self.ws_client.state.is_running
            else kwargs.get(
                "node",
                self.state.api_node,
            )
    ...
    def _connection_latency(self) -> int:
        ...
        return self.http_client.latency
