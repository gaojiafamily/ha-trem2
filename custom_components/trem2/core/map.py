...
TW_MAP_SVG = {
    "head": """
        ...
        <style>@font-face {{ font-family: 'NotoSansTC'; src: url(data:font/ttf;base64,{font_base64}) format('truetype'); }}</style>
...
def draw(eq_data: dict, intensitys: dict, bg_path=None, url=None, font_path=None):
    ...
    # Append SVG head
    svg_parts.append(
        TW_MAP_SVG["head"].format(
            font_base64=get_font_data(font_path),
        )
    )
...
def get_font_data(file_path) -> str:
    """Get the font data as a base64 encoded string."""
    font_path = Path(file_path)

    if not font_path.exists():
        return ""

    font_cont = font_path.read_bytes()
    return base64.b64encode(font_cont).decode("utf-8")
